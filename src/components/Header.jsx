...
const socialLinks = [
  ...
  { id: 2, icon: <AiFillInstagram />, url: "https://www.instagram.com/" },
...
const iconsPages = [
  { id: 1, icon: <FaSearch />, url: "/search" },
  { id: 2, icon: <FaHeart />, url: "/favorite" },
  { id: 3, icon: <FaUserAlt />, url: "/user" },
  { id: 4, icon: <FaShoppingCart />, url: "/cart" },
];
...
export default function Header() {
  ...
  return (
    <header>
        ...
        <Container>
          <nav className="flex items-center justify-between h-16">
            ...
            <button
              ...
              className="md:hidden flex flex-col gap-1 cursor-pointer hover:rotate-90 transition-transform duration-300"
            >
              <span className="h-1 w-6 bg-primary-400 rounded-sm" />
              <span className="h-1 w-6 bg-primary-400 rounded-sm" />
              <span className="h-1 w-6 bg-primary-400 rounded-sm" />
            </button>
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <li
                    key={link.id}
                    className={`
                      capitalize pb-1 border-b-2 transition-all duration-300
                      ${
                        isActive
      ...
      {/* Mobile Menu */}
      {showMobileMenu && (
        <div className="fixed md:hidden top-0 right-0 w-1/2 h-screen bg-primary-50 flex items-center justify-center text-center z-50">
          <nav className="flex flex-col gap-6">
            <button
              onClick={() => setShowMobileMenu(false)}
              className="absolute top-4 left-4 text-5xl text-primary-400"
            >
              <IoIosCloseCircle />
            </button>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  to={link.url}
                  onClick={() => setShowMobileMenu(false)}
                  className={`capitalize text-lg transition-colors duration-300
                    ${
                      isActive
